...
sequencer is implemented in Rust under `crates/`, with 53 passing tests and an
...
**Gate-A STARK work has started.** `crates/circuit-spend` is a real Plonky3 FRI
STARK over BabyBear + Poseidon2 (the frozen proof-system choices): it proves the
spend statement's Poseidon2 workload and its sound arithmetic constraints, and
reports the Gate-A KPIs (prove time, proof size, verify throughput).
...
deferred (the fused full-statement circuit, DAG-BFT consensus, persistence/P2P
backends).
...
- [`docs/gate-a-report.md`](docs/gate-a-report.md) — measured Gate-A KPIs (prove time, proof size,
  verify throughput) for the spend statement's STARK components, generated by `gate-a-bench`.
